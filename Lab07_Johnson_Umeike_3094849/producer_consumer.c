...
void *producer (void *parg)
{
  ...
  while (1) {
    /*
     * Do work to produce an item. The get a slot in the queue for
    ...
    /*sem_getvalue(fifo->slotsToPut,sval);
    if( *sval == YOU_WILL_DETERMINE_FOR_PRODUCERS)
      printf ("prod %d:   %s.\n", my_tid, status);
    else*/
    printf("prod %d:  %d.\n", my_tid, item);
    sem_post(fifo->slotsToGet);
...
void *consumer (void *carg)
{
  ...
  while (1) {
    ...
    /*sem_getvalue(fifo->slotsToGet,sval);
    if( *sval == YOU_WILL_DETERMINE_FOR_CONSUMERS)
      printf ("con %d:   %s.\n", my_tid, status);
    else*/
    printf ("con %d:   %d.\n", my_tid, item);
    sem_post(fifo->slotsToPut);
