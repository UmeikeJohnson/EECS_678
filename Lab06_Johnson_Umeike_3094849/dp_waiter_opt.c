...
typedef struct {
  ...
  pthread_t      thread;       /* Thread structure for this
                                  philosopher */
  int            count;        /* count the number of times a philospher
                                  is awakened but both chopsticks are not free */
} philosopher;
...
static void *dp_thread(void *arg)
{
  ...
  while (!Stop) {
    ...
    while(*left_chop_available(me) != 1 && *right_chop_available(me) != 1) {
      pthread_cond_wait(&(me->can_eat),&waiter);
      me->count++;
      //if(count == )
    }
    //printf("\ncount for process %d: %d\n",me->id,me->count);
    //pthread_mutex_lock(left_chop(me));
    //pthread_mutex_lock(right_chop(me));
    
    // mark my chopsticks as taken
...
void set_table()
{
  int i;
    ...
    /*
   * Initialize all the Mutexes that represent the chopsticks in the ASYMMETRIC SOLUTION.
   * NOT NEEDED FOR WAITER SOLUTION
   */
...
void print_progress()
{
  ...
  for (i = 0; i < NUM_PHILS;) {
    ...
    for (j = 0; j < 4; j++) {
       ...
       */
      sprintf(buf, "%d/%d | %d", 
              Diners[i].prog, Diners[i].prog_total, Diners[i].count);
