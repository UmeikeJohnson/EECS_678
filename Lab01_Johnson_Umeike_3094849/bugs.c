...
struct Storage {
  ...
  char* sentence[7]; // It might be necessary to grow this array. Johnson: increased from 6 to 7 
  //to accommodate the NULL character to fulfil condition in echo routine
...
void echoohce(char** strs) {
  ...
  for (iter = strs; *iter != NULL; ++iter)
    printf("%s ", *iter); //Johnson: changed from iter to *iter to match type "%s"
  ...
  // wrong.
  for (; iter != stop_beginning; --iter) //Johnson: I used stop_beginning as the loop control condition instead of NULL
    printf("%s ", *iter);
...
int main(int argc, char** argv) {
  ...
  bug_info.num_bugs_on_mars = 0;   //Johnson: this should be initialized here
  ...
  free(bug_info.sentence[2]); //Johnson: free up space on heap before another dynamic
  //allocation by strdup() to prevent memory leakage
  ...
  printf("The current bug adjective is: %s\n", bug_info.sentence[6]); //Johnson: change bug_info.sentence[2] to bug_info.sentence[6] to print desired output of (null)
  ...
  //free(bug_info.useless_bug); // Remove this line if it is problematic Johnson: no need to free this since it isn't dynamically allocated. This means bug_info.useless_bug is on the stack
